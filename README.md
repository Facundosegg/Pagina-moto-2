...
### Paso 5.12 — Estadísticas por moto (vistas y consultas)

Esto le muestra a cada cliente, dentro de su propio panel de "Cargar
motos", qué motos generan más interés: cuántas veces se abrió la
ficha grande de cada una, y cuántas veces alguien tocó "Consultar".
Cada cliente ve solo las estadísticas de su propio catálogo.

**1. Creá la tabla que guarda los eventos:**

```sql
create table moto_eventos (
  id bigint generated by default as identity primary key,
  moto_id bigint references motos(id) on delete cascade,
  cliente_id uuid references clientes(id) on delete cascade,
  tipo text not null check (tipo in ('vista', 'consulta')),
  created_at timestamptz not null default now()
);

alter table moto_eventos enable row level security;

create policy "Cualquiera puede registrar un evento"
on moto_eventos for insert
to public
with check ( true );

create policy "El admin de cada cliente ve sus propias estadísticas"
on moto_eventos for select
to authenticated
using (
...
Igual que con las fotos de "Vendé tu moto" que planteamos antes,
acá también hace falta permitir que **cualquiera** (sin login) pueda
insertar un evento — es la única forma de registrar la actividad de
un visitante que no está logueado. No es información sensible (solo
"alguien miró esta moto"), y nadie puede leer estos datos salvo el
administrador correspondiente a cada cliente (o vos, como
superadmin) — un visitante no puede consultar las estadísticas de
nadie, solo generar las suyas propias.

**2. Subí el código actualizado a GitHub** como siempre y esperá el
redeploy. No hace falta ninguna clave nueva.

**Cómo se ve:** dentro del panel de "Cargar motos" de cada cliente,
arriba del todo (antes del listado del catálogo), hay una sección
plegable **"Estadísticas — qué motos generan más interés"**. Al
abrirla, muestra todas las motos ordenadas de la más consultada a la
menos, con la cantidad de vistas y de consultas de cada una.

Los eventos se registran solos, sin que el visitante note nada: al
tocar "Consultar" en cualquier parte del sitio (tarjeta, ficha
grande, o el formulario "Pedí tu moto" cuando eligen una moto
puntual), y al abrir la ficha grande de una moto.

### Paso 5.13 — Que los clientes puedan recuperar su contraseña solos

Esto agrega un link **"¿Olvidaste tu contraseña?"** en la pantalla de
login de cada cliente. Al tocarlo, pone su email, Supabase le manda un
mail con un link, y al tocarlo llega a una pantalla para elegir una
contraseña nueva — sin que vos tengas que resetearla a mano.

**No hace falta ninguna clave nueva.** Un solo paso de configuración:

1. En `src/data.js`, revisá la constante `RESET_PASSWORD_URL` — tiene
   que apuntar a **cualquiera de tus dominios reales**, con
   `/?recover=1` al final. Ejemplo:
   `https://pagina-moto-22026.vercel.app/?recover=1`. Da igual cuál
   cliente use el link para recuperar su contraseña — la pantalla de
   "elegir contraseña nueva" es la misma para todos, así que con un
   solo dominio fijo alcanza (no hay que agregar uno por cada cliente).

2. En Supabase: **Authentication → URL Configuration**, en
   **"Redirect URLs"** agregá esa misma URL exacta (con `/?recover=1`
   al final). Si no la agregás ahí, Supabase rechaza el link por
   seguridad.

3. Subí el código a GitHub como siempre y esperá el redeploy.

**Una limitación para tener en cuenta:** Supabase manda estos mails
con su propio servicio de correo por defecto, que en el plan gratis
tiene un límite bastante bajo de mails por hora (pensado para
pruebas, no para uso real con varios clientes). Para las primeras
pruebas alcanza, pero si esto crece y varios clientes piden recuperar
su contraseña seguido, conviene configurar un proveedor de correo
propio (Supabase permite conectar uno gratis, como Resend) en
**Authentication → Settings → SMTP Settings** — es un paso aparte,
opcional, para cuando haga falta.
